...
TARGETS = [
    ...
    },
    {
        ...
        "url": (
            "https://www.river.go.jp/kawabou/pcfull/tm"
            "?itmkndCd=4&ofcCd=5377&obsCd=63&isCurrent=true&fld=0"
        ),
    },
]
...
def resolve_full_date(date_str: str, now_dt: datetime.datetime) -> datetime.date:
    """
    「MM/DD」形式の日付文字列(年情報なし)から、実行時点に近い年を推測して
    実際の datetime.date を求める。
    """
    try:
        _month_str, day_str = date_str.split("/")
        day = int(day_str)
    except (ValueError, AttributeError):
        return now_dt.date()

    year, month = resolve_year_month(date_str, now_dt)
    return datetime.date(year, month, day)


def get_week_bounds(d: datetime.date):
    """dが含まれる週の(月曜日, 日曜日)のdateを返す"""
    monday = d - datetime.timedelta(days=d.weekday())
    sunday = monday + datetime.timedelta(days=6)
    return monday, sunday


def get_output_csv_path_for_week(monday: datetime.date, sunday: datetime.date, name: str) -> str:
    """週の月曜日・日曜日・地点名を指定してCSVパスを組み立てる"""
    filename = f"{name}-{monday:%Y-%m-%d}_{sunday:%Y-%m-%d}.csv"
...
def make_week_sort_key(monday: datetime.date, sunday: datetime.date):
    """
    特定の週(月曜〜日曜)の中でのソートキーを作る。

    CSV内の日付は年なし(MM/DD)なので、年末年始をまたぐ週(例: 12/29〜1/4)
    では月だけでは新旧が正しく判定できない。その週のmonday/sundayの年を
    手がかりに、月からどちらの年に属するかを判定して正しく並べる。
    """
    def sort_key(key):
        ...
        try:
            ...
            return (0, 0, 0, 0, 0)
        year = monday.year if month == monday.month else sunday.year
...
def process_target(target: dict, now_dt: datetime.datetime, now: str) -> None:
    ...
    # スクレイプ結果を「行の実際の日付が属する週(月曜始まり)」に基づいて
    # 週ごとのバケツに振り分ける。
    # bucket_key = 月曜日のdate -> { key: {"取得日時":..., "values":[...]} }
    buckets = {}
    bucket_headers = {}
    bucket_bounds = {}
    new_counts = {}
    ...
    def get_bucket(monday: datetime.date, sunday: datetime.date):
        bucket_key = monday
        if bucket_key not in buckets:
            path = get_output_csv_path_for_week(monday, sunday, name)
            records, existing_header = load_existing_records(path)
            buckets[bucket_key] = records
            bucket_headers[bucket_key] = existing_header
            bucket_bounds[bucket_key] = (monday, sunday)
    ...
    for t_idx, table in enumerate(tables):
        ...
        for row in data_rows:
            if date_idx is not None and time_idx is not None and len(row) > max(date_idx, time_idx):
                ...
                full_date = resolve_full_date(last_date, now_dt)
                monday, sunday = get_week_bounds(full_date)
                bucket_key = monday
                records = get_bucket(monday, sunday)
    ...
    for bucket_key, records in buckets.items():
        monday, sunday = bucket_bounds[bucket_key]
        ...
        sort_key = make_week_sort_key(monday, sunday)
        sorted_keys = sorted(records.keys(), key=sort_key, reverse=True)
